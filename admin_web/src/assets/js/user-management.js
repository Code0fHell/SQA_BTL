...
async function deleteUser(id) {
  try {
    ...
    alert("Không thể xóa người dùng vì: " + error.message);
  }
}
...
confirmAddUserButton.addEventListener("click", async () => {
  ...
  if (currentEditUserId) { // Nếu đang chỉnh sửa
    try {
      ...
      } else if (error.message.includes("Username is already")) {
        alert("Tên người dùng đã tồn tại. Vui lòng sử dụng tên người dùng khác.");
      } else {
        alert("Không thể cập nhật người dùng vì: " + error.message);
      }
    }
    ...
    try {
      ...
      alert("Không thể thêm người dùng vì: " + error.message);
    }
    // Đóng modal và đặt lại các trường
    $("#addUserModal").modal("hide");
    document.getElementById("addUserForm").reset(); // Đặt lại form
